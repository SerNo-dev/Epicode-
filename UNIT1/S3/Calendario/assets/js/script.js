...
const showAppointments = (indexOfTheDay) => {
    ...
    //3. cliclare gli appuntamenti del giorno e creare gli elementi di lista 
    appointmentsList.innerHTML = ''; // prima la svuoto
...
const createDays = (numberOfDays) => {
    ...
    for (let i = 0; i < numberOfDays; i++) {
        ...
        //Dentro al ciclo Creo i div dei singoli giorni
        // 1. scrivo il numero del giorno 
        const cellValue = document.createElement('h3');
        cellValue.innerText = i + 1; // cosi scrive i numeri dei giorni
        dayCellDiv.appendChild(cellValue) // cosi scrivo il numero del giorno
        //2. appendo il div del singolo giorno al calendar
        calendarDiv.appendChild(dayCellDiv);
        // Popolo l array con l array corrispondente al singolo giorno,cioè creo un array dentro un array
        appointments.push([]); //Crea l array corrispondente a quel giorno ovviamento vuoto

    }
    console.log(appointments);// dovremmo trovare un array con 29 array vuoti
}
createDays(dayInThisMonth()); // chiamo una funzione dentro una funzione
...
const handleFormSubmit = (e) => {
    ...
    //in unica stringa scrivo orario e testo dell'impegno
    const meetingString = `${meetingTime} - ${meetingName}`;
    //scrivo la stringa nell'array dentro appointments corrispondente al giorno
    const indiceGiorno = parseInt(selectedDay) - 1; //con parse int trasformo il testo(perche il numero del giorno arriva come testo) in numero e faccio -1 perche devo cercare l indice che sta una poszione sotto al giorno effettivo(arrray)
    appointments[indiceGiorno].push(meetingString);

    // voglio indicare con un pallino i giorni nei quali ci sono impegni
    const dot = document.createElement('span');
    dot.classList.add('dot');
    //selezione la cella del giorno cliccato 
    const selectedCell = document.querySelector('.selected');
    // Se il giorno ha già il pallino,non lo devo aggiungere
    if (!selectedCell.querySelector('.dot')) { // solo se non trovi un elemento con la classe dot lo aggiungi altrimenti no
