  ...
  public Page<Blog> getBlogPosts(int page, int size, String sortBy) {
    ...
    public String patchCoverBlogPost(int id, MultipartFile cover) throws IOException {
        ...
        if (blogOpt.isPresent()) {
            ...
            Blog blogpost = blogOpt.get();
            blogpost.setCover(url);
            blogPostRepository.save(blogpost);
            return "Blog-Post con id = " + id + "aggiornato correttamente con la cover inviata";
        } else {
            throw new PostNonTrovatoException("Post con di = " + id + "non trovato");
        }
    }
